...
sp1=0.9
sp2=1.1
always_include_prefix=true

. utils/parse_options.sh

if [ $# != 2 ]; then
  echo "Usage: perturb_data_dir_speed_3way.sh <srcdir> <destdir>"
  echo "Applies standard 3-way speed perturbation using factors of ${sp1}, 1.0 and ${sp2}."
  echo "e.g.:"
...
utils/data/perturb_data_dir_speed.sh ${sp1} ${srcdir} ${destdir}_speed${sp1} || exit 1
utils/data/perturb_data_dir_speed.sh ${sp2} ${srcdir} ${destdir}_speed${sp2} || exit 1
...
if $always_include_prefix; then
  ...
  utils/data/combine_data.sh $destdir ${destdir}_speed1.0 ${destdir}_speed${sp1} ${destdir}_speed${sp2} || exit 1

  rm -r ${destdir}_speed${sp1} ${destdir}_speed${sp2} ${destdir}_speed1.0
else
  utils/data/combine_data.sh $destdir ${srcdir} ${destdir}_speed${sp1} ${destdir}_speed${sp2} || exit 1
  rm -r ${destdir}_speed${sp1} ${destdir}_speed${sp2}
fi
